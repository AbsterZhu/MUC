...
class Human36M(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.smplx_path = osp.join(cfg.data_dir, 'Human36M', 'SMPL-X')
        ...
        self.joint_set = {'joint_num': 17,
                        ...
                        'regressor': np.load(cfg.human36m_J17_regressor)
                        }
    ...
    def load_data(self):
        ...
        for subject in subject_list:
            ...
            with open(osp.join(self.smplx_path, 'Human36M_subject' + str(subject) + '_SMPLX_NeuralAnnot.json'),'r') as f:
        ...
        for aid in tqdm(db.anns.keys()):
            ...
            if str(subject) not in all_data_dict.keys():
                all_data_dict[str(subject)] = {}
            if str(action_idx) not in all_data_dict[str(subject)].keys():
                all_data_dict[str(subject)][str(action_idx)] = {}
            if str(subaction_idx) not in all_data_dict[str(subject)][str(action_idx)].keys():
                all_data_dict[str(subject)][str(action_idx)][str(subaction_idx)] = {}
            if str(frame_idx) not in all_data_dict[str(subject)][str(action_idx)][str(subaction_idx)].keys():
                all_data_dict[str(subject)][str(action_idx)][str(subaction_idx)][str(frame_idx)] = {}
            ...
            all_data_dict[str(subject)][str(action_idx)][str(subaction_idx)][str(frame_idx)][str(cam_idx)] = {
                'img_path': img_path,
                'img_shape': img_shape,
                'bbox': bbox,
                'joint_img': joint_img,
        ...
        for subject in all_data_dict.keys():
            for action_idx in all_data_dict[subject].keys():
                for subaction_idx in all_data_dict[subject][action_idx].keys():
                    for frame_idx in all_data_dict[subject][action_idx][subaction_idx].keys():
                        temp_data_list = []
                        for cam_idx in all_data_dict[subject][action_idx][subaction_idx][frame_idx].keys():
                            data = all_data_dict[subject][action_idx][subaction_idx][frame_idx][cam_idx]
                            temp_data_list.append(data)
                        multi_view_data_list.append(temp_data_list)

        if self.data_split == 'train':
            print('[Human36M train] original size:', len(db.anns.keys()) // 4,
    ...
    def __getitem__(self, idx):
        ...
        for cam_index, data in enumerate(multi_view_data):
            ...
            inputs = {'img': img}
            targets = {'joint_img': smplx_joint_img, 
                       'smplx_joint_img': smplx_joint_img,
                       'joint_cam': smplx_joint_cam_wo_ra, 
            ...
            # v = smplx_mesh_cam_orig * 1000
            # mesh = trimesh.Trimesh(vertices=v, faces=smpl_x.face)
            # mesh.export("fuck.obj")
...
def fusionDataloader():
    train_set = DataLoader(
        Human36M(transforms.ToTensor(), 'train'),
    ...
    val_set = DataLoader(
        Human36M(transforms.ToTensor(), 'test'),
